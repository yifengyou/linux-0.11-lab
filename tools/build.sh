...
dd if=$bootsect bs=512 count=1 of=$IMAGE status=none 2>&1 >/dev/null

# Write setup(4 * 512bytes, four sectors)
[ ! -f "$setup" ] && echo "Error: No setup binary file there" && exit -1
dd if=$setup seek=1 bs=512 count=4 of=$IMAGE status=none 2>&1 >/dev/null
...
dd if=$kernel seek=5 bs=512 of=$IMAGE status=none 2>&1 >/dev/null
